...
<html>
  <head>
	...
	<script type="text/javascript">
		$(function(){
			...
			var newManagers = "<%=session.getAttribute("newManagers")%>";
			if(newManagers != null && newManagers != "null" && newManagers != ""){
			...
			var nowPage = "<%=session.getAttribute("createNowPage")%>";
		...
		function firstPage(){
			...
			window.location.href = "transport/managerManagementAction!showCreatePage?batchNowPage=1";
			return;
		}
		function prevPage(){
			var nowPage = $("#nowPageInput").val();
			...
			nowPage--;
			window.location.href = "transport/managerManagementAction!showCreatePage?batchNowPage=" + nowPage;
			return;
		}
		function nextPage(){
			var nowPage = $("#nowPageInput").val();
		...
		function lastPage(){
			...
			window.location.href = "transport/managerManagementAction!showCreatePage?batchNowPage=" + allPage;
			return;
		}
		function jumpTo(){
			var nowPage = $("#nowPageInput").val();
		...
		function createAll(){
			if(confirm('确认全部创建吗？')){
				window.location.href = "transport/managerManagementAction!createAll";
			}
		}
		function createSelect(){
			...
			for(var i = 0; i<checks.length; i++){
				...
				if(isCheck){
					...
					if(i < checks.length -1){
						createAccounts += $(checks[i]).next().val() + ",";
					}else{
						createAccounts += $(checks[i]).next().val();
					}
				}
			...
			if(confirm('确认创建勾选的账号吗？')){
				window.location.href = "transport/managerManagementAction!createSelect?createAccounts=" + createAccounts;
			}
		}
  ...
  <body>
  	...
  	<div id="inputPwdDiv">
		<form id="inputPwdForm" action="transport/managerManagementAction!uploadFile" method="post">
    ...
    <div id="uploadDiv">
    	<form action="transport/managerManagementAction!uploadFile"
			enctype="multipart/form-data" id="fileForm" method="post" onsubmit="return importFile()">
				...
				<label id="friendNote">友情提示：</label><label id="friendNote1">数据库中账号已存在和所属驾校不存在的新用户将不会被显示</label>
		</form>
    </div>
    ...
    <div id="userListDiv">
    	<s:if test="#session.newManagers != null">
    	...
    	<label>&emsp;&emsp;共“${sessionScope.createTotalNum}”条</label>&emsp;&emsp;<label>共“${sessionScope.createPageNum}”页</label>
    	<table>
				<thead>
					<tr id="titleTr">
						<th>勾选</th>
				...
				<tbody>
					<%int i= session.getAttribute("createNowPage") != null && ((Integer)session.getAttribute("createNowPage")) != 0 ? 0+((Integer)session.getAttribute("createNowPage")-1)*10 : 0; %>
					<s:iterator value="#session.newManagers" var= "u">
						<tr class="contentTr">
							<td><input class="userCheck" type="checkbox" /><input type="hidden" value='<s:property value="#u.school_manager_account" />' /></td>
							<td><%=++i%></td>
							<td><s:property value="#u.school_manager_account" /></td>
							<td><s:property value="#u.school_manager_password" /></td>
							...
							<td>
								<a class="operate" href="transport/managerManagementAction!createManagerDetail?createAccount=<s:property value="#u.school_manager_account" />">详情</a>
							</td>
						</tr>
					</s:iterator>
				</tbody>
