...
<script type="text/javascript">
	...
	function prevPage(){
		...
		nowPage--;
		window.location.href = "transport/managerJurisdictionAction!showPage?nowPage=" + nowPage;
		return;
	}
	function nextPage(){
		var nowPage = $("#nowPageInput").val();
	...
	function lastPage(){
		...
		window.location.href = "transport/managerJurisdictionAction!showPage?nowPage=" + allPage;
		return;
	}
	function jumpTo(){
		var nowPage = $("#nowPageInput").val();
	...
	function reviseSelect(){
		...
		for(var i = 0; i<checks.length; i++){
			...
			if(isCheck){
				...
				if(i < checks.length -1){
					userAccounts += $(checks[i]).next().val() + ",";
				}else{
					userAccounts += $(checks[i]).next().val();
				}
			}
		...
		window.location.href = "transport/managerJurisdictionAction!alterJurs?userAccounts=" + userAccounts;
	
	}
	function showAll(){
		var nowPage = $("#nowPageInput").val();
		if(!nowPage.match("^[0-9]*$")) {
			window.location.href = "transport/managerJurisdictionAction!showPage?nowPage=1&showAll=true";
			return;
		}
		var allPage = "<%= request.getAttribute("allPage")%>";
		if(nowPage < 1){
			...
			window.location.href = "transport/managerJurisdictionAction!showPage?nowPage=" + allPage + "&showAll=true";
			return;
		}
		window.location.href = "transport/managerJurisdictionAction!showPage?nowPage=" + nowPage  + "&showAll=true";
	}
	...
	function orderByAccount(){
		var nowPage = $("#nowPageInput").val();
		window.location.href = "transport/managerJurisdictionAction!showPage?nowPage=" + nowPage + "&orderType=school_manager_account";
	}
	function orderByName(){
		var nowPage = $("#nowPageInput").val();
		window.location.href = "transport/managerJurisdictionAction!showPage?nowPage=" + nowPage + "&orderType=school_manager_name";
	}
	function orderBySchool(){
		var nowPage = $("#nowPageInput").val();
		window.location.href = "transport/managerJurisdictionAction!showPage?nowPage=" + nowPage + "&orderType=school_id";
	}
	function orderByState(){
		var nowPage = $("#nowPageInput").val();
		window.location.href = "transport/managerJurisdictionAction!showPage?nowPage=" + nowPage + "&orderType=school_manager_state";
	}
	function orderByDefault(){
		var nowPage = $("#nowPageInput").val();
		window.location.href = "transport/managerJurisdictionAction!showPage?nowPage=" + nowPage + "&orderType=defaultOrder";
	}
</script>
</head>
...
<body>
	<div id="searchDiv">
		<!-- 搜索用户的模块 -->
		<form action="transport/managerJurisdictionAction!searchUser"
			...
			<div id="userStateDiv" class="smallDiv">
				<label>账号状态：</label> <select name="searchUser.school_manager_state"
					id="searchState">
					<option>全部</option>
					<option>可用</option>
					<option>冻结</option>
	...
	<div id="userListDiv">
		...
		<div id="listFunctionDiv">
			...
			<input type="button" value="批量修改" class="btn"
				onclick="reviseSelect()" />&emsp; <input type="button" value="≤"
				class="btn1" id="firstPageBtn" onclick="firstPage()" /> <input
				type="button" value="＜" class="btn1" id="prevPageBtn"
				onclick="prevPage()" /> <input type="button" value="＞" class="btn1"
				id="nextPageBtn" onclick="nextPage()" /> <input type="button"
				value="≥" class="btn1" id="lastPageBtn" onclick="lastPage()" /> <input
				type="text" value="1" class="page" id="nowPageInput"
				onblur="changePage()" /> <input type="button" value="跳转至"
				class="btn" onclick="jumpTo()" /> <label>&emsp;共“<s:property
					value="allPage" />”页
					...
					<s:iterator value="managerUsers" var="u">
						<tr class="contentTr">
							<td><input class="userCheck" type="checkbox" /><input
								type="hidden"
								value="<s:property value="#u.school_manager_account" />" /></td>
							<td><%=++i%></td>
							<td><s:property value="#u.school_manager_account" /></td>
							<td><s:property value="#u.school_manager_name" /></td>
							<td
								title="驾校所属地：<s:property value="#u.school.school_province" />&emsp;<s:property value="#u.school.school_city" />&emsp;<s:property value="#u.school.school_address" />&#10;驾校状态：<s:property value="#u.school.school_state" />"><s:property
									value="#u.school.school_name" /></td>
							<td><s:property value="#u.school_manager_state" /></td>
							<td><a class="operate"
								href="transport/managerJurisdictionAction!showJurisdictionDetailPage?managerAccount=<s:property value="#u.school_manager_account" />">查看</a>
								<a class="operate"
								href="transport/managerJurisdictionAction!showJurisdictionAlterPage?managerAccount=<s:property value="#u.school_manager_account" />">修改</a>
							</td>
						</tr>
					</s:iterator>
				</tbody>
