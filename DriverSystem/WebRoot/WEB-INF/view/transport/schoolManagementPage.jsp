...
<script type="text/javascript">
	...
	function prevPage(){
		...
		nowPage--;
		window.location.href = "transport/schoolManagementAction!showPage?nowPage=" + nowPage;
		return;
	}
	function nextPage(){
		var nowPage = $("#nowPageInput").val();
	...
	function lastPage(){
		...
		window.location.href = "transport/schoolManagementAction!showPage?nowPage=" + allPage;
		return;
	}
	function jumpTo(){
		var nowPage = $("#nowPageInput").val();
	...
	function showAll(){
		var nowPage = $("#nowPageInput").val();
		if(!nowPage.match("^[0-9]*$")) {
			window.location.href = "transport/schoolManagementAction!showPage?nowPage=1&showAll=true";
			return;
		}
		var allPage = "<%= request.getAttribute("allPage")%>";
		if(nowPage < 1){
			...
			window.location.href = "transport/schoolManagementAction!showPage?nowPage=" + allPage + "&showAll=true";
			return;
		}
		window.location.href = "transport/schoolManagementAction!showPage?nowPage=" + nowPage  + "&showAll=true";
	}
	...
	function deleteSelect(){
		...
		if(confirm('确认删除吗？')){
			window.location.href = "transport/schoolManagementAction!deleteSelectSchool?schoolIds=" + schoolIds;	
		}
	}
	...
	function createBatch(){
		window.location.href = "transport/schoolManagementAction!createBatchPage";
	
	}
	function orderByName(){
		var nowPage = $("#nowPageInput").val();
		window.location.href = "transport/schoolManagementAction!showPage?nowPage=" + nowPage + "&orderType=school_name";
	}
	function orderByProvince(){
		var nowPage = $("#nowPageInput").val();
		window.location.href = "transport/schoolManagementAction!showPage?nowPage=" + nowPage + "&orderType=school_province";
	}
	function orderByCity(){
		var nowPage = $("#nowPageInput").val();
		window.location.href = "transport/schoolManagementAction!showPage?nowPage=" + nowPage + "&orderType=school_city";
	}
	function orderByState(){
		var nowPage = $("#nowPageInput").val();
		window.location.href = "transport/schoolManagementAction!showPage?nowPage=" + nowPage + "&orderType=school_state";
	}
	function orderByDefault(){
		var nowPage = $("#nowPageInput").val();
		window.location.href = "transport/schoolManagementAction!showPage?nowPage=" + nowPage + "&orderType=defaultOrder";
	}
</script>
</head>
...
<body>
	<div id="searchDiv"><!-- 搜索用户的模块 -->
		<form action="transport/schoolManagementAction!searchSchool" method="post" enctype="application/x-www-form-urlencoded" >
			...
			<div id="schoolStateDiv" class="smallDiv">
				<label>驾校状态：</label> 
				<select name="searchSchool.school_state" id="searchState">
					<option>全部</option>
					<option>未审核</option>
					<option>审核通过</option>
					<option>审核拒绝</option>
	...
	<div id="userListDiv"><!-- 用户列表的模块 -->
		<div id="listFunctionDiv"><!-- 全选分页的模块 -->
				...
				<label>全选/反选</label>
				<input type="button" value="录入驾校" class="btn" onclick="createSchool()"/>
				<input type="button" value="批量录入" class="btn" onclick="createBatch()"/>
				...
				<tbody>
					...
					<s:iterator value="schoolList" var= "u">
						<tr class="contentTr">
							...
							<td>
								<a class="operate" href="transport/schoolManagementAction!deleteSchool?schoolId=<s:property value="#u.school_id" />" onclick="return deleteSchool(this)">删除</a>
								<input type="hidden" value="<s:property value="#u.school_state" />" />
								<a class="operate" href="transport/schoolManagementAction!alterSchoolPage?schoolId=<s:property value="#u.school_id" />" >修改</a>
								<a class="operate" href="transport/schoolManagementAction!schoolDetailPage?schoolId=<s:property value="#u.school_id" />" >详情</a>
							</td>
						</tr>
					</s:iterator>
				</tbody>
